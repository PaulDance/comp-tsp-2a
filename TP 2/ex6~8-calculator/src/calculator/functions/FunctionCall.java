...
public abstract class FunctionCall extends Expression {
	...
	public FunctionCall(final List<Expression> argsList, int expectedArgsCount) throws IllegalArgumentException {
		super(argsList.toArray(new Expression[0]));
		this.argsList = argsList;
		this.expectedArgsCount = expectedArgsCount;
		final int argc = argsList.size();
		
		if (this.expectedArgsCount != ANY_ARGS_COUNT) {
			if (this.expectedArgsCount == AT_LEAST_ONE_ARG) {
				if (argc < 1) {
					throw new IllegalArgumentException(this.getName() + " expects at least one argument, "
														+ argc + " given instead.");
				}
			}
			else if (this.expectedArgsCount != argc) {
				throw new IllegalArgumentException(this.getName() + " expects "
													+ this.expectedArgsCount + " argument(s), "
													+ argc + " given instead.");
			}
		}
	}
