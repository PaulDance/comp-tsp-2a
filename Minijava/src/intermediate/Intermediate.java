...
public class Intermediate extends ASTVisitorDefault {
    ...
    private final IR ir;
    /**
     * Attribut synthétisé nodeVar = Variable IR Temp pour résultat des
     * expression.
     */
    ...
    public void visit(final ExprArrayLookup exprArrayLookup) {
        ...
        this.add(new QAssignArrayFrom(this.getVar(exprArrayLookup.array),
                this.getVar(exprArrayLookup.index), this.getVar(exprArrayLookup)));
    }
    ...
    public void visit(final ExprArrayNew exprArrayNew) {
        ...
        this.add(new QNewArray(this.newTemp(), this.getVar(exprArrayNew.size),
    ...
    public void visit(final ExprCall exprCall) {
        ...
        this.add(new QCall(this.newLabel(exprCall.methodId.name),
                this.newConst(exprCall.args.size() + 1), this.getVar(exprCall)));
    }
    ...
    public void visit(final StmtArrayAssign stmtArrayAssign) {
        ...
        this.add(new QAssignArrayTo(this.getVar(stmtArrayAssign.value),
                this.getVar(stmtArrayAssign.index), this.getVar(stmtArrayAssign)));
    }
    ...
    private IRVar getVar(final ASTNode node) {
        return this.varAttr.get(node);
    }

    private IRVar setVar(final ASTNode node, final IRVar var) {
    ...
    private IRVar newLabel(final String name) {
        return this.ir.newLabel(name);
    }

    private IRVar newConst(final int value) {
     ...
     */
    private IRVar lookupVar(final String name, final ASTNode node) {
