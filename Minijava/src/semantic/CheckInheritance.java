...
public class CheckInheritance {
    ...
    public CheckInheritance(final SemanticTree semanticTree) {
        ...
        if (this.error) {
            DEBUG.logErr(
                    "Error(s) in CheckInheritance : Java inheritance unmanaged for some classes");
        }
    }

    // ...
    private InfoKlass parent(final InfoKlass kl) { // heritage sur InfoKlass
    ...
    private void checkAndBuild() {
        ...
        for (final InfoKlass kl: this.rootScope.getKlasses()) {
    ...
    private boolean checkObject() {
        ...
        return this.assume(this.objKlass != null, "Missing Object Class")
                && this.assume(this.objKlass.getParent() == null,
                        "Object Class extends !! " + this.objKlass);
    }

    private boolean checkAncestors(final InfoKlass kl) {
        // branche sans boucles, de classes connues, et "Object" comme racine
        final HashSet<InfoKlass> ancestors = new HashSet<>();
        boolean ok = true;
