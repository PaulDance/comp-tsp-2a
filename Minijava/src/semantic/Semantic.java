...
public class Semantic {
    ...
    private void analyse() {
        ...
        final BuildSymTab bst = new BuildSymTab(this.semanticTree);
        ...
        final CheckInheritance cch = new CheckInheritance(this.semanticTree);
        ...
        final TypeChecking tc = new TypeChecking(this.semanticTree);
        this.error = tc.getError() || this.error;

        final CheckUndefinedSymbols cUs = new CheckUndefinedSymbols(this.semanticTree);
        this.error = cUs.getError() || this.error;

        final CheckUnusedSymbols cus = new CheckUnusedSymbols(this.semanticTree);
        this.error = cus.getError() || this.error;

        if (DEBUG.UNUSED) {
            for (final InfoVar unusedVar: cus.unusedVars) {
                System.out.println(
                        String.format("Warning: unused variable or attribute '%s'.", unusedVar));
            }
        }
    }
}
