...
public class BuildSymTab extends ASTVisitorDefault {
    ...
    /**
     * Attribut hérité, sauvegardé dans semabticTree.nodeScope. == entrée dans
     * la table des symboles pour chaque noeud.
     */
    ...
    public void visit(final Klass klass) {
        ...
        final InfoKlass infoKlass = new InfoKlass(klass.klassId.name, klass.parentId.name);
    ...
    public void visit(final Method method) {
        ...
        for (final ASTNode node: method.fargs) {
        ...
        this.currentScope = this.newMethodScope(this.currentScope,
                new InfoMethod(method.returnType.name, method.methodId.name, varsArray));
    ...
    public void visit(final Var var) {
        ...
        final InfoVar v = new InfoVar(var.varId.name, var.typeId.name);
     ...
     */
    private Scope newKlassScope(Scope sc, final InfoKlass kl) {
     ...
     */
    private Scope newMethodScope(Scope sc, final InfoMethod m) {
    ...
    /**
     * Gestion des redéfinitions dans une même portée. NB : HashMap.add() non
     * null => already exists.
     */
